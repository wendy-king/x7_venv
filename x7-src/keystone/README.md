# Keystone: X7 Identity Service

Keystone is a Python implementation of the [X7](http://www.x7.org) identity service API.

# Documentation

## For users and sysadmins

Learn how to install, configure, manage, and interact with the X7
Identity Service API at the [X7 Documentation](http://docs.x7.org/) site.

## For contributors

Learn how to setup a development environment and then test, run, and contribute to Keystone at the
[Contributor Documentation](http://keystone.x7.org/) site.
...
* Search for information in the archives of the [X7 mailing list](http://wiki.x7.org/MailingLists), or post a question.
* Ask a question in the [#x7 IRC channel](http://wiki.x7.org/UsingIRC).
...
* RemoteAuth  - WSGI middleware that can be used in services (like Chase, Engine, and Tank) when Auth middleware is running remotely
...
[X7 Manuals project](https://launchpad.net/x7-manuals).
...
Initial support for using keystone as engine's identity component has been started.

    # clone projects
    bzr clone lp:engine
    git clone git://github.com/x7/keystone.git

    # link keystone into the engine root dir
    ln -s keystone/keystone engine/keystone

    # run engine-api based on the paste config in keystone
    engine/bin/engine-api --api_paste_config=keystone/examples/paste/engine-api-paste.ini

Assuming you added the test data using bin/sampledata, you can then use joeuser/secrete

## Chase Integration - Quick Start

1.  Install Chase, either from trunk or version 1.4.1 (once it's released) or
    ...
    this quick start; see http://chase.x7.org/development_saio.html for
    a Chase development set up guide. Once you have a working Chase install, go
    ahead and shut it down for now (the default Chase install uses the same
...
2.  Obtain and install a source copy of Keystone:
        ...
        $ git clone https://github.com/x7/keystone.git ~/keystone
        ...
...
4.  In another window, edit the `~/keystone/keystone/test/sampledata.py` file,
    find the `chase.publicinternets.com` text and replace it with the URL to
    your Chase cluster using the following format (note that we're going to
    change Chase to run on port 8888 later):
...
6.  Reconfigure Chase's proxy server to use Keystone instead of TempAuth.
    Here's an example `/etc/chase/proxy-server.conf`:
...
7.  Start Chase back up with the new configuration:

        $ chase-init main start
        ...

8.  Use `chase` to check everything works (note: you currently have to create a
    container or upload something as your first action to have the account
    created; there's a Chase bug to be fixed soon):

        $ chase -A http://127.0.0.1:5000/v1.0 -U joeuser -K secrete post container
        $ chase -A http://127.0.0.1:5000/v1.0 -U joeuser -K secrete stat -v
