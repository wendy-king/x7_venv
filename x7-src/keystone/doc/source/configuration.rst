...
Once configured, users and tenants need to be defined so that other X7
...
in Keystone and used (typically configured into) Engine, Chase, Tank, and any
other X7 projects, to be able to use Keystone services.
...
configuring Engine, Tank, or other X7 services.

Defining Services and Service Endpoints
#######################################

Keystone also acts as a service catalog to let other X7 systems know
where relevant API endpoints exist for X7 Services. The X7
Dashboard, in particular, uses this heavily - and this **must** be configured
for the X7 Dashboard to properly function.
...
between X7 components - where the 'public facing' API may not be
available.

An example of setting up the endpoint for Engine::

    $> keystone-manage endpointTemplates add RegionOne engine \
    http://engine-api.mydomain:8774/v1.1/ \
    http://engine-api.mydomain:8774/v1.1/ \
    http://engine-api.mydomain:8774/v1.1/ \
    1 1
...
Chase::
    ...
    http://chase.mydomain:8080/v1/AUTH_[TENANT_ID] \
    http://chase.mydomain:8080/v1.0/ \
    http://chase.mydomain:8080/v1/AUTH_[TENANT_ID] \
    1 1
...
Configuring Engine to use Keystone
################################

To configure Engine to use Keystone for authentication, the Engine API service
can be run against the api-paste file provided by Keystone. This is most
easily accomplished by setting the `--api_paste_config` flag in engine.conf to
point to `examples/paste/engine-api-paste.ini` from Keystone. This paste file
included references to the WSGI authentication middleware provided with the
keystone installation.

When configuring Engine, it is important to create a admin service token for
the service (from the Configuration step above) and include that as the key
'admin_token' in the engine-api-paste.ini. See the documented
:doc:`engine-api-paste` file for references.

Configuring Chase to use Keystone
#################################

Similar to Engine, chase can be configured to use Keystone for authentication
rather than it's built in 'tempauth'.

1. Add a service endpoint for Chase to Keystone

2. Configure the paste file for chase-proxy (`/etc/chase/chase-proxy.conf`)

3.  Reconfigure Chase's proxy server to use Keystone instead of TempAuth.
...
5. Verify that keystone is providing authentication to Chase

Use `chase` to check everything works (note: you currently have to create a
