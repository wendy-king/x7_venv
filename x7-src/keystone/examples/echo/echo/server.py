...
"""
Echo: a dummy service for X7 auth testing. It returns request info.
"""
...
class EchoApp(object):
    ...
    def toDOM(self, environ):
        echo = etree.Element("{http://docs.x7.org/echo/api/v1.0}echo",
                             method=environ["REQUEST_METHOD"],
                             pathInfo=environ["PATH_INFO"],
                             queryString=environ.get('QUERY_STRING', ""))
        content = etree.Element(
            "{http://docs.x7.org/echo/api/v1.0}content")
...
if __name__ == "__main__":
    def usage():
        print "Runs Echo, the canonical X7 service, " \
