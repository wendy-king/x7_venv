...
What we add to the request for use by the X7 service
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
class AuthProtocol(object):
    ...
    def _init_protocol_common(self, app, conf):
        ...
        # where to find the X7 service (if not in local WSGI chain)
        ...
        self.service_url = '%s://%s:%s' % (self.service_protocol,
                                           self.service_host,
                                           self.service_port)
        # used to verify this component with the X7 service or PAPIAuth
