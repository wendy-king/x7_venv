...
class TestServer(unittest.TestCase):
    ...
    def test_send_result_xml(self):
        ...
        user = xml.find("{http://docs.x7.org/identity/api/v2.0}user")
        token = xml.find("{http://docs.x7.org/identity/api/v2.0}token")
    ...
    def test_get_normalized_request_content_xml(self):
        ...
        body = '<?xml version="1.0" encoding="UTF-8"?> \
                <auth xmlns="http://docs.x7.org/identity/api/v2.0">\
                <passwordCredentials \
                xmlns="http://docs.x7.org/identity/api/v2.0" \
