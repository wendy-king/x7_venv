...
from chase.common import manager
...
class TestServer(unittest.TestCase):
    ...
    def join_chase_dir(self, path):
        return os.path.join(manager.CHASE_DIR, path)
    ...
    def test_create_server(self):
        ...
        self.assertEquals(server.cmd, 'chase-proxy-server')
        ...
        self.assertEquals(server.cmd, 'chase-object-replicator')
    ...
    def test_get_conf_file_name(self):
        ...
        conf_file = self.join_chase_dir(server_name + '.conf')
    ...
    def test_conf_files(self):
        ...
        with temptree(conf_files) as t:
            ...
            proxy_conf = self.join_chase_dir('proxy-server.conf')
        ...
        with temptree(conf_files) as t:
            ...
            c1 = self.join_chase_dir('object-server1.conf')
            c2 = self.join_chase_dir('object-server/2.conf')
            c3 = self.join_chase_dir('object-server/object3.conf')
            c4 = self.join_chase_dir('object-server/conf/server4.conf')
        ...
        with temptree(conf_files) as t:
            ...
            self.assertEquals(conf_file,
                              self.join_chase_dir('account-server/2.conf'))
        ...
        with temptree(conf_files) as t:
            ...
            try:
                with open(os.path.join(t, 'output'), 'w+') as f:
                    ...
                    c1 = self.join_chase_dir('container-server/1.conf')
        ...
        with temptree(conf_files) as t:
            ...
            self.assertEquals(len(conf_files), 1)
            conf_file = conf_files[0]
            conf = self.join_chase_dir(server_name + '.conf')
    ...
    def test_iter_pid_files(self):
        ...
        with temptree(conf_files) as chase_dir:
            manager.CHASE_DIR = chase_dir
    ...
    def test_get_running_pids(self):
        ...
        with temptree(*zip(*pid_files)) as t:
            ...
            pid_two = self.join_chase_dir('test-server2.pid')
            ...
            with temptree(files, pids) as t:
                ...
                try:
                    with open(os.path.join(t, 'output'), 'w+') as f:
                        ...
                        line = output[0]
                        self.assert_('test-server running' in line)
                        conf_four = self.join_chase_dir(conf_files[3])
                        ...
                        conf_three = self.join_chase_dir(conf_files[2])
    ...
    def test_spawn(self):
        ...
        with temptree(['test-server.conf']) as chase_dir:
            manager.CHASE_DIR = chase_dir
            with temptree([]) as t:
                manager.RUN_DIR = t
                ...
                try:
                    ...
                    conf_file = self.join_chase_dir('test-server.conf')
                    ...
                    conf1 = self.join_chase_dir('test-server/1.conf')
                    conf2 = self.join_chase_dir('test-server/2.conf')
                    conf3 = self.join_chase_dir('test-server/3.conf')
                    conf4 = self.join_chase_dir('test-server/4.conf')
                    server = manager.Server('test')
                    ...
                    self.assertEquals(len(server.procs), 1)
                    proc = server.procs[0]
                    expected_args = ['chase-test-server', conf1, 'once']
                    # assert stdout is piped
                    self.assertEquals(proc.stdout, MockProcess.PIPE)
                    self.assertEquals(proc.stderr, proc.stdout)
                    ...
                    proc = server.procs[1]
                    expected_args = ['chase-test-server', conf2, 'verbose']
                    self.assertEquals(proc.args, expected_args)
                    ...
                    proc = server.procs[3]
                    expected_args = ['chase-test-server', conf4, 'once',
                                     'verbose']
                    self.assertEquals(proc.args, expected_args)
            ...
            with temptree(files, pids) as t:
                ...
                try:
                    with open(os.path.join(t, 'output'), 'w+') as f:
                        ...
                        server.spawn = mock_spawn
                        conf1 = self.join_chase_dir('object-server/1.conf')
                        conf2 = self.join_chase_dir('object-server/2.conf')
                        conf3 = self.join_chase_dir('object-server/3.conf')
                        conf4 = self.join_chase_dir('object-server/4.conf')
                        expected = {
                            1: conf1,
                            2: conf2,
                            3: conf3,
                        ...
                        self.assert_('chase-auth-server does not exist' in
                                     pop_stream(f))
                finally:
                    sys.stdout = old_stdout
        ...
        with temptree(conf_files) as chase_dir:
            ...
            with temptree(files, pids) as t:
                ...
                for pid in (1, 2, 3, 4):
                    ...
                    self.assertEquals(manager.os.pid_sigs[pid],
                                      [signal.SIGTERM])
                conf1 = self.join_chase_dir('account-reaper/1.conf')
                conf2 = self.join_chase_dir('account-reaper/2.conf')
                conf3 = self.join_chase_dir('account-reaper/3.conf')
                conf4 = self.join_chase_dir('account-reaper/4.conf')
...
class TestManager(unittest.TestCase):
    ...
    def test_start(self):
        ...
        old_chase_server = manager.Server
        try:
            ...
            manager.Server = old_chase_server
    ...
    def test_no_wait(self):
        ...
        orig_chase_server = manager.Server
        try:
            manager.Server = MockServer
            # test success
            ...
            manager.Server = orig_chase_server
