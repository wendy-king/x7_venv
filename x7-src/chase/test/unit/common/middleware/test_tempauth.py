...
from chase.common.middleware import tempauth as auth
...
class FakeApp(object):
    ...
    def __call__(self, env, start_response):
        ...
        if self.sync_key:
            self.request.environ['chase_sync_key'] = self.sync_key
        if 'chase.authorize' in env:
            resp = env['chase.authorize'](self.request)
            if resp:
...
class TestAuth(unittest.TestCase):
    ...
    def test_auth_deny_non_reseller_prefix_no_override(self):
        ...
        resp = self._make_request('/v1/BLAH_account',
            ...
            environ={'chase.authorize': fake_authorize}
            ).get_response(self.test_auth)
        self.assertEquals(resp.status_int, 500)
        self.assertEquals(resp.environ['chase.authorize'], fake_authorize)
    ...
    def test_reseller_admin_is_owner(self):
        ...
        def mitm_authorize(req):
            ...
            owner_values.append(req.environ.get('chase_owner', False))
            return rv
