...
from chase.common.middleware import ratelimit
from chase.proxy.server import get_container_memcache_key
from chase.common.memcached import MemcacheConnectionError
...
class TestRateLimit(unittest.TestCase):
    ...
    def test_restarting_memcache(self):
        ...
        req.method = 'PUT'
        req.environ['chase.cache'] = FakeMemcache()
        req.environ['chase.cache'].error_on_incr = True
        make_app_call = lambda: self.test_ratelimit(req.environ,
                                                    start_response)
        begin = time.time()
        self._run(make_app_call, num_calls, current_rate, check_time=False)
