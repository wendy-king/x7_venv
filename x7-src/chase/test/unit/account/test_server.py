...
from chase.account.server import AccountController, ACCOUNT_LISTING_LIMIT
from chase.common.utils import normalize_timestamp


class TestAccountController(unittest.TestCase):
    """ Test chase.account_server.AccountController """
    def setUp(self):
        """ Set up for testing chase.account_server.AccountController """
        self.testdir = os.path.join(os.path.dirname(__file__), 'account_server')
        self.controller = AccountController(
            {'devices': self.testdir, 'mount_check': 'false'})

    def tearDown(self):
        """ Tear down for testing chase.account_server.AccountController """
        try:
            rmtree(self.testdir)
        except OSError, err:
