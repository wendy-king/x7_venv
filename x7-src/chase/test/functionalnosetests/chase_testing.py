...
if conf:
    ...
    try:
        ...
        pass  # skip
    if 'account' in conf:
        chase_test_user[0] = '%(account)s:%(username)s' % conf
    else:
        chase_test_user[0] = '%(username)s' % conf
    chase_test_key[0] = conf['password']
    try:
        chase_test_user[1] = '%s%s' % ('%s:' % conf['account2'] if 'account2'
                                       in conf else '', conf['username2'])
        chase_test_key[1] = conf['password2']
    except KeyError, err:
        pass  # old conf, no second account tests can be run
    try:
        chase_test_user[2] = '%s%s' % ('%s:' % conf['account'] if 'account'
                                       in conf else '', conf['username3'])
        chase_test_key[2] = conf['password3']
    except KeyError, err:
        pass  # old conf, no third account tests can be run

skip = not all([chase_test_auth, chase_test_user[0], chase_test_key[0]])
if skip:
    print >>sys.stderr, 'SKIPPING FUNCTIONAL TESTS DUE TO NO CONFIG'

skip2 = not all([not skip, chase_test_user[1], chase_test_key[1]])
if not skip and skip2:
    print >>sys.stderr, \
          'SKIPPING SECOND ACCOUNT FUNCTIONAL TESTS DUE TO NO CONFIG FOR THEM'

skip3 = not all([not skip, chase_test_user[2], chase_test_key[2]])
