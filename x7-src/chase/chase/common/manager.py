...
from chase.common.utils import search_tree, remove_file, write_file

CHASE_DIR = '/etc/chase'
RUN_DIR = '/var/run/chase'
...
class Server():
    ...
    def get_pid_file_name(self, conf_file):
        ...
        return conf_file.replace(
            os.path.normpath(CHASE_DIR), RUN_DIR, 1).replace(
    ...
    def get_conf_file_name(self, pid_file):
        ...
        if self.server in STANDALONE_SERVERS:
            return pid_file.replace(
                os.path.normpath(RUN_DIR), CHASE_DIR, 1).rsplit(
                        '.pid', 1)[0] + '.conf'
        else:
            return pid_file.replace(
                os.path.normpath(RUN_DIR), CHASE_DIR, 1).replace(
    ...
    def conf_files(self, **kwargs):
        ...
        """
        if self.server in STANDALONE_SERVERS:
            found_conf_files = search_tree(CHASE_DIR, self.server + '*',
                                           '.conf')
        else:
            found_conf_files = search_tree(CHASE_DIR, '%s-server*' % self.type,
                                          '.conf')
