...
"""Miscellaneous utility functions for use with Chase."""
...
from chase.common.exceptions import LockTimeout, MessageTimeout
...
HASH_PATH_SUFFIX = ''
if hash_conf.read('/etc/chase/chase.conf'):
    try:
        HASH_PATH_SUFFIX = hash_conf.get('chase-hash',
                                         'chase_hash_path_suffix')
...
def validate_configuration():
    if HASH_PATH_SUFFIX == '':
        sys.exit("Error: [chase-hash]: chase_hash_path_suffix missing "
                 "from /etc/chase/chase.conf")
...
class ChaseLogFormatter(logging.Formatter):
    """
...
def get_logger(conf, name=None, log_to_console=False, log_route=None,
    ...
    **Log config and defaults**::
    ...
    if not conf:
        conf = {}
    if name is None:
        name = conf.get('log_name', 'chase')
    if not log_route:
        log_route = name
    logger = logging.getLogger(log_route)
    logger.propagate = False
    # all new handlers will get the same formatter
    formatter = ChaseLogFormatter(fmt)
...
def parse_options(parser=None, once=False, test_args=None):
    """
    Parse standard chase server/daemon options with optparse.OptionParser.
...
def cache_from_env(env):
    ...
    :returns: chase.common.memcached.MemcacheRing from environment
    """
    return item_from_env(env, 'chase.cache')
