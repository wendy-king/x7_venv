...
``/etc/chase/proxy-server.conf`` file just after any auth middleware. Also, the
...
Example usage of this middleware via ``chase``:

    Make the container publicly readable::

        chase post -r '.r:*' container
    ...
    Set an index file directive::
        ...
        chase post -m 'web-index:index.html' container
    ...
    Turn on listings::
        ...
        chase post -m 'web-listings: true' container
    ...
    Enable a custom listings style sheet::
        ...
        chase post -m 'web-listings-css:listings.css' container

    Set an error file::

        chase post -m 'web-error:error.html' container
...
from chase.common.utils import cache_from_env, get_logger, human_readable, \
...
class StaticWeb(object):
    ...
    def _get_escalated_env(self, env):
        ...
        for name in ('eventlet.posthooks', 'chase.trans_id', 'REMOTE_USER',
                     'SCRIPT_NAME', 'SERVER_NAME', 'SERVER_PORT',
                     'SERVER_PROTOCOL', 'chase.cache'):
            if name in env:
