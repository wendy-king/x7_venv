...
from chase.common.utils import get_logger, drop_privileges, \
...
def make_pre_authed_request(env, method, path, body=None, headers=None,
    ...
    for name in ('chase.cache', 'chase.trans_id'):
        if name in env:
            newenv[name] = env[name]
    newenv['chase.authorize'] = lambda req: None
    if not headers:
        headers = {}
    if body:
