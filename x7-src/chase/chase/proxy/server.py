...
# NOTE: chase_conn
# You'll see chase_conn passed around a few places in this file. This is the
...
from chase.common.utils import cache_from_env, ContextPool, get_logger, \
    get_remote_client, normalize_timestamp, split_path, TRUE_VALUES
from chase.common.bufferedhttp import http_connect
from chase.common.constraints import check_metadata, check_object_creation, \
    check_utf8, CONTAINER_LISTING_LIMIT, MAX_ACCOUNT_NAME_LENGTH, \
    MAX_CONTAINER_NAME_LENGTH, MAX_FILE_SIZE
from chase.common.exceptions import ChunkReadTimeout, \
...
def delay_denial(func):
    """
    Decorator to declare which methods should have any chase.authorize call
...
class SegmentedIterable(object):
    """
    Iterable that returns the object contents for a segmented object in Chase.
    ...
    def __init__(self, controller, container, listing, response=None):
        ...
        # See NOTE: chase_conn at top of file about this.
        self.segment_iter_chase_conn = None
        self.position = 0
    ...
    def _load_next_segment(self):
        ...
        try:
            ...
            # See NOTE: chase_conn at top of file about this.
            self.segment_iter_chase_conn = getattr(resp, 'chase_conn', None)
        except StopIteration:
            raise
        except (Exception, Timeout), err:
            if not getattr(err, 'chase_logged', False):
    ...
    def app_iter_range(self, start, stop):
        ...
        try:
            ...
            for chunk in self:
                ...
                yield chunk
            # See NOTE: chase_conn at top of file about this.
            if self.segment_iter_chase_conn:
                try:
                    self.segment_iter_chase_conn.close()
                except Exception:
                    pass
                self.segment_iter_chase_conn = None
...
class Controller(object):
    ...
    def _make_app_iter_reader(self, node, source, queue):
        ...
        try:
            ...
            if getattr(source, 'chase_conn', None):
                try:
                    source.chase_conn.close()
                except Exception:
                    pass
                source.chase_conn = None
                try:
    ...
    def GETorHEAD_base(self, req, server_type, partition, nodes, path,
        ...
        for node in nodes:
            ...
            try:
                ...
                with Timeout(self.app.node_timeout):
                    ...
                    # See NOTE: chase_conn at top of file about this.
                    possible_source.chase_conn = conn
            except (Exception, Timeout):
        ...
        if source:
            if req.method == 'GET' and source.status in (200, 206):
                ...
                # See NOTE: chase_conn at top of file about this.
                res.chase_conn = source.chase_conn
                update_headers(res, source.getheaders())
                # Used by container sync feature
                res.environ['chase_x_timestamp'] = \
...
class ObjectController(Controller):
    ...
    def GETorHEAD(self, req):
        ...
        if 'chase.authorize' in req.environ:
            req.acl = \
                self.container_info(self.account_name, self.container_name)[2]
            aresp = req.environ['chase.authorize'](req)
            if aresp:
                return aresp
    ...
    def PUT(self, req):
        ...
        if 'x-timestamp' in req.headers:
            try:
                ...
                if 'chase_x_timestamp' in hreq.environ and \
                    float(hreq.environ['chase_x_timestamp']) >= \
...
class ContainerController(Controller):
    ...
    def clean_acls(self, req):
        if 'chase.clean_acl' in req.environ:
            for header in ('x-container-read', 'x-container-write'):
                if header in req.headers:
                    try:
                        req.headers[header] = \
                            req.environ['chase.clean_acl'](header,
                                                           req.headers[header])
    ...
    def GETorHEAD(self, req):
        ...
        if 'chase.authorize' in req.environ:
            ...
            if aresp:
                return aresp
        if not req.environ.get('chase_owner', False):
...
class BaseApplication(object):
    ...
    def __init__(self, conf, memcache=None, logger=None, account_ring=None,
        ...
        self.resellers_conf.read(os.path.join(chase_dir, 'resellers.conf'))
        ...
        self.account_ring = account_ring or \
            Ring(os.path.join(chase_dir, 'account.ring.gz'))
        self.memcache = memcache
        mimetypes.init(mimetypes.knownfiles +
                       [os.path.join(chase_dir, 'mime.types')])
    ...
    def handle_request(self, req):
        ...
        try:
            ...
            if 'chase.trans_id' not in req.environ:
                # if this wasn't set by an earlier middleware, set it now
                trans_id = 'tx' + uuid.uuid4().hex
                req.environ['chase.trans_id'] = trans_id
                self.logger.txn_id = trans_id
            req.headers['x-trans-id'] = req.environ['chase.trans_id']
            controller.trans_id = req.environ['chase.trans_id']
            ...
            if 'chase.authorize' in req.environ:
                ...
                # we remove the chase.authorize hook so isn't ever called
                # again. If not authorized, we return the denial unless the
                # controller's method indicates it'd like to gather more
                # information and try again later.
                resp = req.environ['chase.authorize'](req)
                if not resp:
                    # No resp means authorized, no delayed recheck required.
                    del req.environ['chase.authorize']
                else:
...
class Application(BaseApplication):
    ...
    def posthooklogger(self, env, req):
        ...
        self.access_logger.info(' '.join(quote(str(x)) for x in (
                ...
                req.environ.get('chase.trans_id', '-'),
                logged_headers or '-',
                trans_time,
            )))
