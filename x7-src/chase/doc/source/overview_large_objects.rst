...
Chase has a limit on the size of a single uploaded object; by default this is
...
-------------------------------------
Using ``chase`` for Segmented Objects
-------------------------------------

The quickest way to try out this feature is use the included ``chase`` Chase Tool.
You can use the ``-S`` option to specify the segment size to use when splitting
a large file. For example::

    chase upload test_container -S 1073741824 large_file

This would split the large_file into 1G segments and begin uploading those
segments in parallel. Once all the segments have been uploaded, ``chase`` will
then create the manifest file so the segments can be downloaded as one.

So now, the following ``chase`` command would download the entire large object::

    chase download test_container large_file

``chase`` uses a strict convention for its segmented object support. In the above
...
``chase`` will manage these segment files for you, deleting old segments on
...
instead of having ``chase`` do that for you. You can just upload the segments like
...
is useful to keep container listings clean as explained above with ``chase``.
...
* The response's ``ETag`` for a ``GET`` or ``HEAD`` on the manifest file will
  ...
  in the ``<container>/<prefix>`` listing, dynamically. Usually in Chase the
...
The primary factor driving the limitation of object size in chase is
...
Like all of chase, Large Object Support is living feature which will continue
