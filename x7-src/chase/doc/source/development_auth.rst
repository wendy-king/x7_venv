...
The included chase/common/middleware/tempauth.py is a good example of how to
...
the Chase Proxy application, and afterwards when the proxy issues callbacks to
...
Authentication is performed on the request before it ever gets to the Chase
...
authorization procedures need; the Chase Proxy application itself needs no
...
tokens and Chase storage accounts they create with a configurable reseller
...
Chase cluster. Otherwise, the Chase cluster will have to try all the resellers
until one validates a token or all fail.

A restriction with group names is that no group name should begin with a period
'.' as that is reserved for internal Chase use (such as the .r for referrer
...
Example Authentication with TempAuth:
    ...
    * Now this user will have full access (via authorization procedures later)
      to the AUTH_storage_xyz Chase storage account and access to containers in
...
Authorization is performed through callbacks by the Chase Proxy server to the
WSGI environment's chase.authorize value, if one is set. The chase.authorize
...
Adding the chase.authorize callback is often done by the authentication
...
callback before passing on the request. To continue our example above::
    ...
    class Authorization(object):
        ...
        def __call__(self, environ, start_response):
            environ['chase.authorize'] = self.authorize
...
The Chase Proxy server will call chase.authorize after some initial work, but
...
then chase.authorize will be called once more. These are called delay_denial
...
attribute in the webob.Request passed to chase.authorize.
...
The access control string has a standard format included with Chase, though
this can be overridden if desired. The standard format can be parsed with
chase.common.middleware.acl.parse_acl which converts the string into two arrays
...
chase.common.middleware.acl.referrer_allowed function. Checking group access is
...
only allow GETs after a referrer check and any requests after a group check::
    ...
    from chase.common.middleware.acl import parse_acl, referrer_allowed
    from webob.exc import HTTPForbidden, HTTPUnauthorized
...
X-Container-Read and X-Container-Write headers. Chase allows these strings to
...
To support this validation, the Chase Proxy application will call the WSGI
environment's chase.clean_acl callback whenever one of these headers is to be
...
There is an included chase.common.middleware.acl.clean_acl that validates the
standard Chase format. Let's improve our example by making use of that::

    from chase.common.middleware.acl import \
    ...
    class Authorization(object):
        ...
        def __call__(self, environ, start_response):
            environ['chase.authorize'] = self.authorize
            environ['chase.clean_acl'] = clean_acl
            return self.app(environ, start_response)
...
folks a start on their own code if they want to use repoze.what::
    ...
    from chase.common.bufferedhttp import http_connect_raw as http_connect
    from chase.common.middleware.acl import clean_acl, parse_acl, referrer_allowed
    from chase.common.utils import cache_from_env, split_path
    from webob.exc import HTTPForbidden, HTTPUnauthorized
