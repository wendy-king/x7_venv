...
Chase is designed to run on commodity hardware. At Rackspace, our storage
...
required and not recommended. Chase's disk usage pattern is the worst
...
The chase services run completely autonomously, which provides for a lot of
flexibility when architecting the hardware deployment for chase. The 4 main
services are:
...
nothing wrong with doing this, as it scales each service out steertally.
...
proxies more manageable.  Storage Services scale out steertally as storage
...
designing the network for a Chase cluster.
...
You can now start building the ring with::
    ...
    chase-ring-builder <builder_file> create <part_power> <replicas> <min_part_hours>
...
Devices can be added to the ring with::
    ...
    chase-ring-builder <builder_file> add z<zone>-<ip>:<port>/<device_name>_<meta> <weight>
...
Once all of the devices are added to the ring, run::
    ...
    chase-ring-builder <builder_file> rebalance
...
chase-ring-builder with no options will display help text with available
...
Chase uses paste.deploy (http://pythonpaste.org/deploy/) to manage server
...
value. Again, weird, but not harmful since Chase just treats the two sets of
...
special distinction with Chase.
...
The main rule to remember when working with Chase configuration files is:
...
------------------  ----------  ---------------------------------------------
chase_dir           /etc/chase  Chase configuration directory
...
lookups, such as auth tokens, and container/account existence.  Chase does
...
Time may be relative but it is relatively important for Chase!  Chase uses
...
Chase is designed to be mostly filesystem agnostic--the only requirement
...
headroom. We do not recommend running Chase on RAID, but if you are using
...
For a standard chase install, all data drives are mounted directly under
...
Rackspace currently runs Chase on Ubuntu Server 10.04, and the following
...
Chase is set up to log directly to syslog. Every service can be configured
