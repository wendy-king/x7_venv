...
The Chase cluster operator must allow synchronization with a set of hosts
...
----------------------------------------------------------
Using the ``chase`` tool to set up synchronized containers
----------------------------------------------------------
...
accounts::
    ...
    $ chase -A http://cluster1/auth/v1.0 -U test:tester -K testing stat -v
    ...
    $ chase -A http://cluster2/auth/v1.0 -U test2:tester2 -K testing2 stat -v
...
we'll make next::
    ...
    $ chase -A http://cluster1/auth/v1.0 -U test:tester -K testing post \
...
something similar for the second cluster's container::
    ...
    $ chase -A http://cluster2/auth/v1.0 -U test2:tester2 -K testing2 post \
...
as it gets synchronized over to the second::
    ...
    $ chase -A http://cluster1/auth/v1.0 -U test:tester -K testing \
      upload container1 .
    photo002.png
    photo004.png
    photo001.png
    ...
    $ chase -A http://cluster2/auth/v1.0 -U test2:tester2 -K testing2 \
      list container2

    [Nothing there yet, so we wait a bit...]
    [If you're an operator running SAIO and just testing, you may need to
     run 'chase-init container-sync once' to perform a sync scan.]
...
So what's ``chase`` doing behind the scenes? Nothing overly complicated. It
...
The chase-container-sync does the job of sending updates to the remote
container.
