...
@register.inclusion_tag('steer/_nav_list.html', takes_context=True)
def steer_main_nav(context):
    """ Generates top-level dashboard navigation entries. """
    if 'request' not in context:
        return {}
    current_dashboard = context['request'].steer.get('dashboard', None)
    dashboards = []
    for dash in Steer.get_dashboards():
...
@register.inclusion_tag('steer/_subnav_list.html', takes_context=True)
def steer_dashboard_nav(context):
    """ Generates sub-navigation entries for the current dashboard. """
    if 'request' not in context:
        return {}
    dashboard = context['request'].steer['dashboard']
    ...
    return {'components': non_empty_panels,
            'user': context['request'].user,
            'current': context['request'].steer['panel'].slug}
