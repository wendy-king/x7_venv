...
"""
General-purpose decorators for use with Steer.
"""
import functools
import logging
...
from steer.exceptions import NotAuthorized
...
def _current_component(view_func, dashboard=None, panel=None):
    ...
    def dec(request, *args, **kwargs):
        if dashboard:
            request.steer['dashboard'] = dashboard
        if panel:
            request.steer['panel'] = panel
...
def require_roles(view_func, required):
    ...
    Example usage::
        ...
        from steer.decorators import require_roles
    ...
    Raises a :exc:`~steer.exceptions.NotAuthorized` exception if the
