...
"""
Contains the core classes and functionality that makes Steer what it is.
This module is considered internal, and should not be relied on directly.

Public APIs are made available through the :mod:`steer` module and
...
from steer.decorators import require_roles, _current_component
...
class SteerComponent(object):
    def __init__(self):
        super(SteerComponent, self).__init__()
        if not self.slug:
...
class Panel(SteerComponent):
    """ A base class for defining Steer dashboard panels.

    All Steer dashboard panels should extend from this class. It provides
    ...
    def get_absolute_url(self):
        ...
        """
        try:
            return reverse('steer:%s:%s:%s' % (self._registered_with.slug,
                                                 self.slug,
                                                 self.index_url_name))
        except:
...
class Dashboard(Registry, SteerComponent):
    """ A base class for defining Steer dashboards.

    All Steer dashboards should extend from this base class. It provides the
    appropriate hooks for automatic discovery of :class:`~steer.Panel`
    ...
    def get_panel(self, panel):
        """
        Returns the specified :class:`~steer.Panel` instance registered
        with this dashboard.
        """
    ...
    def get_panels(self):
        """
        Returns the :class:`~steer.Panel` instances registered with this
        dashboard in order.
        """
    ...
    def get_absolute_url(self):
        ...
        in the URLconf for the :class:`~steer.Panel` specified by
        :attr:`~steer.Dashboard.default_panel`.
        """
        try:
...
class Site(Registry, SteerComponent):
    """ The core X7 Dashboard class. """
    ...
    @property
    def _conf(self):
        conf = copy.copy(STEER_CONFIG)
        conf.update(getattr(settings, 'STEER_CONFIG', {}))
        return conf
    ...
    def register(self, dashboard):
        """ Registers a :class:`~steer.Dashboard` with Steer."""
        return self._register(dashboard)

    def unregister(self, dashboard):
        """ Unregisters a :class:`~steer.Dashboard` from Steer. """
    ...
    def get_dashboard(self, dashboard):
        """ Returns the specified :class:`~steer.Dashboard` instance. """
        return self._registered(dashboard)

    def get_dashboards(self):
        """ Returns an ordered tuple of :class:`~steer.Dashboard` modules.

        Orders dashboards according to the ``"dashboards"`` key in
        ``settings.STEER_CONFIG`` or else returns all registered dashboards
        in alphabetical order.

        Any remaining :class:`~steer.Dashboard` classes registered with
        Steer but not listed in ``settings.STEER_CONFIG['dashboards']``
    ...
    def get_default_dashboard(self):
        """ Returns the default :class:`~steer.Dashboard` instance.

        If ``"default_dashboard"`` is specified in ``settings.STEER_CONFIG``
        then that dashboard will be returned. If not, the first dashboard
        returned by :func:`~steer.get_dashboards` will be returned.
        """
    ...
    def get_user_home(self, user):
        ...
        the ``"user_home"`` key in ``settings.STEER_CONFIG``. Each of these
        would be valid::
    ...
    def get_absolute_url(self):
        """ Returns the default URL for Steer's URLconf.

        The default URL is determined by calling
        :meth:`~steer.Dashboard.get_absolute_url`
        on the :class:`~steer.Dashboard` instance returned by
        :meth:`~steer.get_default_dashboard`.
        """
    ...
    def _urls(self):
        """ Constructs the URLconf for Steer from registered Dashboards. """
        urlpatterns = self._get_default_urlpatterns()
