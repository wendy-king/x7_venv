...
from steer.dashboards.syspanel.images.forms import (DeleteImage,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                LOG.exception('Error connecting to tank while trying to'
                              'upload image')
                messages.error(request,
                               _("Error connecting to tank: %s") % e.message)
            except tank_exception.Error, e:
                LOG.exception('Tank exception while uploading image')
                messages.error(request,
            ...
            form = UploadImageForm(request.POST)
            return shortcuts.render(request,
                                    'django_engine_syspanel/images/upload.html',
                                    {'form': form})
