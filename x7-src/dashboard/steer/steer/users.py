...
#    under the License.
"""
Classes and methods related to user handling in Steer.
"""
...
def get_user_from_request(request):
    """ Checks the current session and returns a :class:`~steer.users.User`.

    If the session contains user data the User will be treated as
    authenticated and the :class:`~steer.users.User` will have all
    its attributes set.

    If not, the :class:`~steer.users.User` will have no attributes set.

    If the session contains invalid data,
    :exc:`~steer.exceptions.NotAuthorized` will be raised.
    """
...
class User(object):
    """ The main user class which Steer expects.
    ...
    .. attribute:: admin
        ...
        privileges. Internally mapped to :meth:`steer.users.User.is_admin`.
    """
