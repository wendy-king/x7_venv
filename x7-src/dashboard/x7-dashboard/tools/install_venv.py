...
"""
Installation script for the X7 Dashboard development virtualenv.
"""
...
VENV = os.path.join(ROOT, '.steer-venv')
...
def install_django_x7():
    print 'Installing steer module in development mode...'
    path = os.path.join(ROOT, 'steer')
    run_command([WITH_VENV, 'python', 'setup.py', 'develop'], cwd=path)


def print_summary():
    summary = """
 Steer development environment setup is complete.

 To activate the virtualenv for the extent of your current shell session you
 can run:

 $ source .steer-venv/bin/activate
  """
    print summary
