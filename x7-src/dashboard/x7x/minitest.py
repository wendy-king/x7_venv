...
auth = x7x.auth.Auth(management_url='http://%s:8080/v2.0/' % host)
token = auth.tokens.create('1234', 'admin', 'secrete')
print token._info

admin_token = auth.tokens.create('1234', 'admin', 'secrete')
accounts = x7x.extras.Account(auth_token=admin_token.id,
        management_url='http://%s:8081/v2.0' % host)

extras = x7x.extras.Extras(auth_token=token.id,
                                 auth_url='http://%s:8774/v1.1/' % host,
                                 management_url='http://%s:8774/v1.1/' % host)

admin = x7x.admin.Admin(auth_token=token.id,
                              auth_url='http://%s:8774/v1.1/' % host,
                              management_url='http://%s:8774/v1.1/' % host)

compute = x7x.compute.Compute(auth_token=token.id,
                                    auth_url='http://%s:8774/v1.1/' % host,
                                    management_url='http://%s:8774/v1.1/' % host)
